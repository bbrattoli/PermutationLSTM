...
# Source: https://github.com/asanakoy/deeppose_tf
# Code adapted for using LSTM
################################################################################
...
class Alexnet_LSTM(object):
    ...
    def __init__(self, init_model=None, num_classes=2,im_shape=(227, 227, 3), 
                 device_id='/gpu:0', num_layers_to_init=9, 
                 random_init_type=RandomInitType.GAUSSIAN, use_batch_norm=False, 
                 gpu_memory_fraction=None,max_length=20,num_hidden=2048,
                 batch_size = 20, train_conv=True, train_fc=True, 
                 **params):
        """
         Args:
        ...
        if self.num_layers_to_init > 9 or self.num_layers_to_init < 0:
            raise ValueError('Number of layer to init must be in [0, 9] ({} provided)'.
        ...
        with tf.variable_scope('input'):
            ...
            self.l_true = tf.placeholder(tf.int64, shape=(None,),name='l_true')
        ...
        with tf.device(self.device_id):
            ...
            with tf.variable_scope('conv1'):
                ...
                tr_vars['conv1w'], tr_vars['conv1b'] = \
                    self.get_conv_weights(layer_index, net_data,
                                          ...
                                          kernels_num, trainable)
                layer_index += 1
                conv1_in = self.conv(self.x, tr_vars['conv1w'], tr_vars['conv1b'],
                                        kernel_height, kernel_width,
            ...
            with tf.variable_scope('conv2'):
                ...
                tr_vars['conv2w'], tr_vars['conv2b'] = \
                    self.get_conv_weights(layer_index, net_data,
                                          ...
                                          kernels_num, trainable)
                layer_index += 1
                conv2_in = self.conv(maxpool1, tr_vars['conv2w'], tr_vars['conv2b'],
                                        kernel_height, kernel_width,
                                        kernels_num, s_h, s_w, padding="SAME", group=group)
            ...
            with tf.variable_scope('conv3'):
                ...
                tr_vars['conv3w'], tr_vars['conv3b'] = \
                    self.get_conv_weights(layer_index, net_data,
                                          ...
                                          kernels_num, trainable)
                layer_index += 1
                conv3_in = self.conv(maxpool2, tr_vars['conv3w'], tr_vars['conv3b'],
                                        kernel_height, kernel_width,
                                        kernels_num, s_h, s_w, padding="SAME", group=group)
            ...
            with tf.variable_scope('conv4'):
                ...
                tr_vars['conv4w'], tr_vars['conv4b'] = \
                    self.get_conv_weights(layer_index, net_data,
                                          ...
                                          kernels_num, trainable)
                layer_index += 1
                conv4_in = self.conv(conv3, tr_vars['conv4w'], tr_vars['conv4b'],
                                        kernel_height, kernel_width,
                                        kernels_num, s_h, s_w, padding="SAME", group=group)
            ...
            with tf.variable_scope('conv5'):
                ...
                tr_vars['conv5w'], tr_vars['conv5b'] = \
                    self.get_conv_weights(layer_index, net_data,
                                          ...
                                          kernels_num, trainable)
                layer_index += 1
                self.conv5 = self.conv(conv4, tr_vars['conv5w'], tr_vars['conv5b'],
                                          kernel_height, kernel_width,
                                          kernels_num, s_h, s_w, padding="SAME", group=group)
            ...
            with tf.variable_scope('fc6'):
                ...
                tr_vars['fc6w'], tr_vars['fc6b'] = \
                    self.get_fc_weights(layer_index, net_data, num_inputs, 
                                        num_outputs, trainable=self.train_fc)
                layer_index += 1
            ...
            with tf.variable_scope('lstm1'):
                ...
                #transform the input from [batch_size*max_length,num_inputs] to
                #[batch_size,max_length,num_input=4096]
                inp = self.fc7_dropout
                inp_lstm = tf.reshape(inp,[-1,self.max_length,num_inputs])
                
                
                #initialize cell
                cell = tf.nn.rnn_cell.BasicLSTMCell(
                        self.num_hidden,forget_bias = 1.0,state_is_tuple=True)
                #unroll the network, pass the data inp_lstm to it and store the output
                #final state: gives just the last output
                outputs,_ = tf.nn.dynamic_rnn(
                        cell,inp_lstm,sequence_length = self.seq_len,\
                        time_major = False,\
                        dtype=tf.float32)
                
                index = tf.range(0,self.batch_size)*self.max_length + self.seq_len-1
                featAll = tf.gather(tf.reshape(outputs, [-1, self.num_hidden]), index)
                
                self.lstm = featAll #featAll has the dimensionality [nr_seq,nr_hidden_units]
                self.lstm_relu = tf.nn.relu(self.lstm,name='relu')
                
                self.lstm_keep_prob = tf.placeholder_with_default(1.0, tuple(),
                                                                 name='keep_prob_pl')
                self.lstm_dropout = tf.nn.dropout(
                        self.lstm_relu,self.lstm_keep_prob,name='dropout')
                
            # fc(num_classes, relu=False, name='fc8')
            with tf.variable_scope('fc8'):
                num_inputs = int(self.lstm_dropout.get_shape()[1])
                num_outputs = self.num_classes
                tr_vars['fc8w'], tr_vars['fc8b'] = \
                    self.get_fc_weights(layer_index, net_data, num_inputs, num_outputs)
                layer_index += 1
                self.fc8 = tf.add(tf.matmul(self.lstm_dropout, tr_vars['fc8w']), tr_vars['fc8b'],
                                  name='fc')
    ...
    def restore_from_snapshot(self, snapshot_path, num_layers,rest_lstm=False,restore_iter_counter=False):
        """
        ...
        """
        if num_layers > 9 or num_layers < 0:
            raise ValueError('You can restore only 0 to 9 layers.')
        if num_layers == 0:
        ...
        if rest_lstm:
            lstm_variables = [v for v in tf.global_variables() if v.name.startswith('lstm1')]
            vars_to_restore.append(lstm_variables[0])
            vars_to_restore.append(lstm_variables[1])
            vars_names_to_restore.append('lstm1M')
            vars_names_to_restore.append('lstm1B')
    ...
    def get_conv_weights(self, layer_index, net_data, kernel_height, kernel_width,
                         num_input_channels, kernels_num, trainable=True):
        layer_names = ['conv{}'.format(i) for i in xrange(1, 6)] + \
                          ['fc{}'.format(i) for i in xrange(6, 8)] + \
                          ['lstm{}'.format(i) for i in xrange(1,2)] + \
                          ['fc{}'.format(i) for i in xrange(8, 9)]
        wights_std = [0.01] * 5 + [0.005, 0.005, None, 0.01]
        bias_init_values = [0.0, 0.1, 0.0, 0.1, 0.1, 0.1, 0.1,None, 0.0]
        ...
        if layer_index >= self.num_layers_to_init or net_data is None:
            ...
            w = tf.Variable(net_data[l_name]['weights'], name='weight', trainable=trainable)
            b = tf.Variable(net_data[l_name]['biases'], name='bias', trainable=trainable)
        return w, b

    def get_fc_weights(self, layer_index, net_data, num_inputs, num_outputs,
                       wights_std=None, bias_init_value=None, trainable=True):
        if layer_index <= 9:
            ...
            layer_names = ['conv{}'.format(i) for i in xrange(1, 6)] + \
                          ...
                          ['lstm{}'.format(i) for i in xrange(1,2)] + \
                          ['fc{}'.format(i) for i in xrange(8, 9)]
            wights_stds = [0.01] * 5 + [0.005, 0.005, None, 0.01]
            bias_init_values = [0.0, 0.1, 0.0, 0.1, 0.1, 0.1, 0.1, None, 0.0]
            l_name = layer_names[layer_index]
            ...
            if self.random_init_type == self.RandomInitType.GAUSSIAN and \
                    (wights_std is None):
        ...
        if layer_index >= self.num_layers_to_init or net_data is None:
            ...
            w = self.random_weight_variable((num_inputs, num_outputs),
                                            stddev=wights_std, trainable=trainable)
            b = self.random_bias_variable((num_outputs,), value=bias_init_value, trainable=trainable)
        else:
            w = tf.Variable(net_data[l_name]['weights'], name='weight', trainable=trainable)
            b = tf.Variable(net_data[l_name]['biases'], name='bias', trainable=trainable)
        return w, b
    
    def random_weight_variable(self, shape, stddev=0.01, trainable=True):
        """
        stddev is used only for RandomInitType.GAUSSIAN
        """
        if self.random_init_type == self.RandomInitType.GAUSSIAN:
            initial = tf.truncated_normal(shape, stddev=stddev)
            return tf.Variable(initial, name='weight', trainable=trainable)
    ...
    @staticmethod
    def random_bias_variable(shape, value=0.1, trainable=True):
        initial = tf.constant(value, shape=shape)
        return tf.Variable(initial, name='bias', trainable=trainable)
